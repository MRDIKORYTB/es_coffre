...
function coffreMeca(player, prixavant,prixtotal,prixajoute)
  MySQL.Async.execute("UPDATE coffremeca SET `solde`=@prixtotal , identifier = @identifier , lasttransfert = @prixajoute WHERE solde = @prixavant AND id = '3' ",{['@prixtotal'] = prixtotal, ['@identifier'] = player ,['@prixajoute'] = prixajoute,['@prixavant'] = prixavant })

end

function GetSoldeMeca()
  local result = MySQL.Sync.fetchAll("SELECT solde FROM coffremeca WHERE id ='3'")
  return tostring(result[1].solde)
end

function GetCaMeca()
  local result = MySQL.Sync.fetchAll("SELECT ca FROM coffremeca WHERE id ='3'")
  return tostring(result[1].ca)
end

function GetSaleMeca()
  local result = MySQL.Sync.fetchAll("SELECT sale FROM coffremeca WHERE id ='3'")
  return tostring(result[1].sale)
end

function ajoutAmendeToCoffreMeca(solde,sale,ca)
  MySQL.Async.execute("UPDATE coffremeca SET `solde`='@solde',ca='@ca',sale='@sale',identifier=@identifier  WHERE id = '3' ",{['@solde'] = solde, ['@identifier'] = player ,['@ca'] = ca,['@sale'] = sale })

end

function blanchimentMeca(solde,sale,ca)
  MySQL.Async.execute("UPDATE coffremeca SET `solde`='@solde',sale='@sale',identifier=@identifier  WHERE id = '3' ",{['@solde'] = solde, ['@identifier'] = player ,['@sale'] = sale })
