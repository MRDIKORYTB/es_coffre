...
function updateCoffreTaxi(player, prixavant,prixtotal,prixajoute)
  MySQL.Async.execute("UPDATE coffretaxi SET `solde`=@prixtotal , identifier = @identifier , lasttransfert = @prixajoute WHERE solde = @prixavant AND id = '3' ",{['@prixtotal'] = prixtotal, ['@identifier'] = player ,['@prixajoute'] = prixajoute,['@prixavant'] = prixavant })

end

function GetSoldeTaxi()
  local result = MySQL.Sync.fetchAll("SELECT solde FROM coffretaxi WHERE id ='3'")
  return tostring(result[1].solde)
end

function GetCaTaxi()
  local result = MySQL.Sync.fetchAll("SELECT ca FROM coffretaxi WHERE id ='3'")
  return tostring(result[1].ca)
end

function GetSaleTaxi()
  local result = MySQL.Sync.fetchAll("SELECT sale FROM coffretaxi WHERE id ='3'")
  return tostring(result[1].sale)
end

function ajoutAmendeToCoffreTaxi(solde,sale,ca)
  MySQL.Async.execute("UPDATE coffretaxi SET `solde`='@solde',ca='@ca',sale='@sale',identifier=@identifier  WHERE id = '3' ",{['@solde'] = solde, ['@identifier'] = player ,['@ca'] = ca,['@sale'] = sale })

end

function blanchimentTaxi(solde,sale,ca)
  MySQL.Async.execute("UPDATE coffretaxi SET `solde`='@solde',sale='@sale',identifier=@identifier  WHERE id = '3' ",{['@solde'] = solde, ['@identifier'] = player ,['@sale'] = sale })
