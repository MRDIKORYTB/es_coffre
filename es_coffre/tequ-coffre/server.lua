...
function updateCoffreTequ(player, prixavant,prixtotal,prixajoute)
  MySQL.Async.execute("UPDATE coffretequ SET `solde`=@prixtotal , identifier = @identifier , lasttransfert = @prixajoute WHERE solde = @prixavant AND id = '3' ",{['@prixtotal'] = prixtotal, ['@identifier'] = player ,['@prixajoute'] = prixajoute,['@prixavant'] = prixavant })

end

function GetSoldeTequ()
  local result = MySQL.Sync.fetchAll("SELECT solde FROM coffretequ WHERE id ='3'")
  return tostring(result[1].solde)
end

function GetCaTequ()
  local result = MySQL.Sync.fetchAll("SELECT ca FROM coffretequ WHERE id ='3'")
  return tostring(result[1].ca)
end

function GetSaleTequ()
  local result = MySQL.Sync.fetchAll("SELECT sale FROM coffretequ WHERE id ='3'")
  return tostring(result[1].sale)
end

function ajoutAmendeToCoffreTequ(solde,sale,ca)
  MySQL.Async.execute("UPDATE coffretequ SET `solde`='@solde',ca='@ca',sale='@sale',identifier=@identifier  WHERE id = '3' ",{['@solde'] = solde, ['@identifier'] = player ,['@ca'] = ca,['@sale'] = sale })

end

function blanchimentTequ(solde,sale,ca)
  MySQL.Async.execute("UPDATE coffretequ SET `solde`='@solde',sale='@sale',identifier=@identifier  WHERE id = '3' ",{['@solde'] = solde, ['@identifier'] = player ,['@sale'] = sale })
