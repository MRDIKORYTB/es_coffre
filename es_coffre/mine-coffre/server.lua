...
function coffremine(player, prixavant,prixtotal,prixajoute)
  MySQL.Async.execute("UPDATE coffreminer SET `solde`=@prixtotal , identifier = @identifier , lasttransfert = @prixajoute WHERE solde = @prixavant AND id = '3' ",{['@prixtotal'] = prixtotal, ['@identifier'] = player ,['@prixajoute'] = prixajoute,['@prixavant'] = prixavant })

end

function GetSoldeMine()
  local result = MySQL.Sync.fetchAll("SELECT solde FROM coffreminer WHERE id ='3'")
  return tostring(result[1].solde)
end

function GetCaMine()
  local result = MySQL.Sync.fetchAll("SELECT ca FROM coffreminer WHERE id ='3'")
  return tostring(result[1].ca)
end

function GetSaleMine()
  local result = MySQL.Sync.fetchAll("SELECT sale FROM coffreminer WHERE id ='3'")
  return tostring(result[1].sale)
end

function ajoutAmendeToCoffreMine(solde,sale,ca)
  MySQL.Async.execute("UPDATE coffreminer SET `solde`='@solde',ca='@ca',sale='@sale',identifier=@identifier  WHERE id = '3' ",{['@solde'] = solde, ['@identifier'] = player ,['@ca'] = ca,['@sale'] = sale })

end

function blanchimentMine(solde,sale,ca)
  MySQL.Async.execute("UPDATE coffreminer SET `solde`='@solde',sale='@sale',identifier=@identifier  WHERE id = '3' ",{['@solde'] = solde, ['@identifier'] = player ,['@sale'] = sale })
